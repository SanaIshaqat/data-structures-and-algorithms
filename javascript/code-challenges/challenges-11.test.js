...
const totalSum = (input) => {
  // Solution code here...
  const sumtotal = input.reduce((result, sumArray) => {
    return result += sumArray.reduce((res, num2) => {
      return res += num2;
    }, 0);
  }, 0);
  return sumtotal;
...
let findMaleAndFemale = (data) => {
  ...
  const addobj = filteredObject.map((item)=> item.name);
  return addobj.join(' and ');
};
...
let findShortest = (data) => {
  ...
  return data.reduce((result,item)=>{
    ...
    return result = Number(item.height)< Number(result.height)? item : result;
  }).name;
